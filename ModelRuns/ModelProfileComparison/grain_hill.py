...
class GrainHill(CTSModel):
    ...
    def initializer(self, grid_size, report_interval, run_duration,
        ...
        if uplift_duration is None:
            self.uplift_duration = run_duration + 1.0
        else:
    ...
    def run(self):
        ...
        while current_time < self.run_duration:
            ...
            if current_time >= next_uplift:
                ...
                if current_time < self.uplift_duration:
                    next_uplift += min(self.uplift_interval,
                                       self.uplift_duration - current_time)
                else:
                    next_uplift = self.run_duration
