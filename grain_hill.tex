...
As expected, $D_e'$ increases with hillslope gradient (Figure~\ref{diffgrad}). The effective diffusivity approaches an asymptote at 30$^\circ$, representing an angle of repose. The pattern resembles the family of nonlinear flux-gradient curves introduced by \citet{andrews} and explored further by \citet{howard1994detachment} and \citet{roering1999hillslope}. At low gradients, $D_e'$ approaches a value of about 10, which represents the product $a N_a$ in ...

[OK, HERE'S AN ISSUE. ON A ``FLAT'' SLOPE, YOU STILL HAVE AN AVERAGE NUMBER OF FACES EXPOSED AS 2, BECAUSE THE PROTRUDING GRAINS HAVE 3 AND THE RECESSED ONES HAVE ONE. SO BASICALLY YOU HAVE AN AVERAGE DISTURBANCE RATE THAT COMBINES THE RATE FOR 3 WITH THE RATE FOR ONE. SO, CALCULATE THE EXPECTED $N_a(d)$ for this condition...]

[BUT BASICALLY THE POINT IS THAT ONE CAN SET d AND $\delta$ TO ACHIEVE A GIVEN SCALE AND RATE.]
