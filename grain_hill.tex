...
The main message of Figure~\ref{wxinguplift} is that the grain hill model demonstrates an equilibrium adjustment between rock uplift and rock weathering, with the latter controlled by exposed surface area. 

[FROM HERE: well, this all raises the question: if the equilibrium adjusts both fractional cover AND slope surface area, what determines how much of each? Presumably it's about transport too. Also note that there are NO cases with zero soil cover! There is always some cover effect. Likewise, both cover and surface area depend on slope: thus, weathering rate itself is slope-sensitive!!]
