...
[NEXT UP: EFFECTIVE DIFFUSIVITY]

I guess the question here is how does disturbance rate relate to effective diffusivity? This is probably considered in some Furbishy papers, which I should probably re-read. In classic diffusion, you've got diffusivity equal to hop-length variance over characteristic time scale. But here of course it's a bit different because the hop length is biased by the topography, rather than a difference in concentration.
...
\appendix
\section{Transition Rules in the Lattice-Grain Model}    %% Appendix A

\subsection{Elastic Collision Rules}

Figure~\ref{elastic} illustrates the transition rules for motion and elastic grain transitions. The rules encompass the various possible collision geometries, including head-on, oblique, and impact from behind when one grain overtakes another. These transition rules conserve average kinetic energy, because average velocity is set by the motion transition rate and each elastic collision changes directions but not velocities.

\begin{figure*}[t]
\includegraphics{Figures/elastic_collision_rules.pdf}
\caption{Illustration of elastic collision rules. Modified from \citet{tucker2016celllab}.}
\label{elastic}
\end{figure*}

\subsection{Frictional Collision Rules}

Figure~\ref{frictional} illustrates the transition rules for frictional-collision grain transitions. Kinetic energy is not conserved.

\begin{figure*}[t]
\includegraphics{Figures/lattice_grain_friction_rules.pdf}
\caption{Illustration of frictional (inelastic) collision rules. Modified from \citet{tucker2016celllab}.}
\label{frictional}
\end{figure*}

\subsection{Gravity Rules}

Figure~\ref{gravity} illustrates the transition rules for gravitational acceleration. The bottom row shows the ``falling on a slope'' rule, which effectively imposes a 30$^\circ$ angle of repose.

\begin{figure*}[t]
\includegraphics{Figures/lattice_grain_gravity_rules.pdf}
\caption{Illustration of gravitational rules. Modified from \citet{tucker2016celllab}.}
\label{gravity}
\end{figure*}
