...
Our approach starts with a two-dimensional continuous-time stochastic lattice-grain model. The model is described in detail by \citet{tucker2016celllab}; here we present a brief overview of its rules and behavior. The framework is based on the pairwise method developed by Narteau and colleagues \citep{REFS} and applied to problems as diverse as eolian dune dynamics and the core-mantle interface.

In the CTS Lattice-Grain Model, the domain consists of a lattice of hexagonal cells. Each cell is assigned a state...
