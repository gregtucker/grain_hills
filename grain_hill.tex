...
We can derive the effective diffusivity, $D$, from the modeled topography by applying the expected relationship between mean elevation and diffusivity. At steady state, mass balance implies that
\begin{equation}
\mathbf{q}_s = E x
\end{equation}
...
\begin{equation}
\frac{d \eta}{d x} = -\frac{E}{D(x)} x
\end{equation}
Integrating and then averaging over $x$, we can solve for the average elevation, $\overline{\eta}$:
\begin{equation}
\overline{\eta} = -\frac{E}{3 \overline{D}} L^2
\end{equation}
where $L$ is the length of the slope from ridge top to base, and $\overline{D}$ is now the average value of the diffusion-like coefficient along the slope. We can then invert this to find $\overline{D}$:
\begin{equation}
\overline{D} = -\frac{E}{3 \overline{\eta}} L^2
\end{equation}
