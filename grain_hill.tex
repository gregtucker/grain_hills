...
o quick summary of how they are treated here

o what experiments to run? don't need to look again at slope width. might as well simplify to have just soil in between. that leaves two parameters: angle and thickness. angle is already dimensionless; thickness is relative to slope length. so you have three dimensionless params: $d'$, $\beta$, and $T'$. But there's no more steady state except in the case of 
 

\section*{TODO LIST}
\begin{itemize}
\item Make a version of the weathering-collapse script that runs all four cases
\item Construct a folder organization for code repo 
\end{itemize}
